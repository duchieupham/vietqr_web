...
import 'package:VietQR/ecom/bank/frames/ecom_add_bank_frame.dart';
...
import 'package:VietQR/ecom/bank/widgets/ecom_confirm_opt_widget.dart';
import 'package:VietQR/ecom/bank/widgets/ecom_policy_bank_widget.dart';
import 'package:VietQR/ecom/bank/widgets/ecom_select_bank_type_widget.dart';
...
class ECOMAddBankView extends StatefulWidget {
  final String userId;

  const ECOMAddBankView({
    super.key,
...
class _AddBankView extends State<ECOMAddBankView> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: BlocProvider<ECOMBankBloc>(
        create: (BuildContext context) => ECOMBankBloc(),
        child: BlocConsumer<ECOMBankBloc, ECOMBankState>(
          bloc: bankBloc,
          listener: (context, state) {
            if (state is ECOMBankSearchingNameState) {
              DialogWidget.instance.openLoadingDialog();
            }
            if (state is ECOMBankSearchNameSuccessState ||
                state is ECOMBankSearchNameFailedState) {
              Navigator.pop(context);
              _focusNode.unfocus();
            }
            if (state is ECOMBankLoadingInsertState) {
              DialogWidget.instance.openLoadingDialog();
            }
            if (state is ECOMBankCheckNotExistedState) {
              //pop loading
              Navigator.pop(context);
              if (state.isAuthenticated) {
                ...
                bankBloc.add(ECOMBankEventInsertUnauthenticated(dto: dto));
              }
            }
            if (state is ECOMBankCheckFailedState) {
              //pop loading
              Navigator.pop(context);
              DialogWidget.instance
                  .openMsgDialog(title: 'Lỗi', msg: 'Vui lòng thử lại sau');
            }
            if (state is ECOMBankCheckExistedState) {
              //pop loading
              Navigator.pop(context);
              DialogWidget.instance.openMsgDialog(
                  title: 'Không thể thêm/liên kết TK', msg: state.msg);
            }
            if (state is ECOMBankInsertUnauthenticatedSuccessState) {
              //pop loading
              Navigator.pop(context);
            ...
            if (state is ECOMBankInsertUnauthenticatedFailedState) {
              //pop loading
              Navigator.pop(context);
              DialogWidget.instance
                  .openMsgDialog(title: 'Không thể thêm TK', msg: state.msg);
            }
            if (state is ECOMBankReuqestOTPLoadingState) {
              DialogWidget.instance.openLoadingDialog();
            }
            if (state is ECOMBankRequestOTPSuccessState) {
              Navigator.pop(context);
              DialogWidget.instance.openPopup(
                width: 300,
                height: 400,
                child: ECOMConfirmOTPWidget(
            ...
            if (state is ECOMBankRequestOTPFailedState) {
              Navigator.pop(context);
              DialogWidget.instance.openMsgDialog(
            ...
            if (state is ECOMBankConfirmOTPLoadingState) {
              DialogWidget.instance.openLoadingDialog();
            }
            if (state is ECOMBankConfirmOTPSuccessState) {
              // Navigator.pop(context);
              // String bankId =
              ...
              bankBloc.add(ECOMBankEventInsert(dto: dto));
              // } else {
            ...
            if (state is ECOMBankConfirmOTPFailedState) {
              Navigator.pop(context);
              DialogWidget.instance.openMsgDialog(
                title: 'Lỗi',
                msg: state.message,
            ...
            if (state is ECOMBankInsertSuccessfulState) {
              //pop loading
              Navigator.pop(context);
              phoneController.clear();
              nameController.clear();
            ...
            if (state is ECOMBankInsertFailedState) {
              DialogWidget.instance.openMsgDialog(
          ...
          builder: (context, state) {
            ...
            return ECOMAddBankFrame(
              ...
              widget2: Consumer<ECOMBankTypeProvider>(
                builder: (context, provider, child) {
                  return ListView(
                    shrinkWrap: true,
                    children: [
                      Consumer<ECOMBankTypeProvider>(
                        builder: (context, provider, child) {
                          return InkWell(
                            onTap: () {
                              DialogWidget.instance.openPopup(
                                child: BlocProvider<ECOMBankTypeBloc>(
                                  create: (BuildContext context) =>
                                      ECOMBankTypeBloc(),
                                  child: ECOMSelectBankTypeWidget(),
                                ),
                                width: 500,
                                height: 500,
                              );
                        ...
                        child: TextFieldWidget(
                          ...
                          onTapOutside: (value) {
                            if (_focusNode.hasFocus) {
                              ...
                              bankBloc.add(ECOMBankEventSearchName(
                              ...
                              function: () {
                                if (provider.bankType.id.isNotEmpty) {
                                  ...
                                  if (provider.isValidUnauthenticateForm()) {
                                    String bankTypeId =
                                        Provider.of<ECOMBankTypeProvider>(
                                                context,
                                                listen: false)
                                            .bankType
                                            .id;
                                    bankBloc.add(
                                      ECOMBankCheckExistedEvent(
                            ...
                            if (provider.bankType.status == 1) ...[
                              ...
                              ButtonWidget(
                                ...
                                function: () async {
                                  ...
                                  provider.updateNationalErr(
                                      !(nationalController.text.length >= 9 &&
                                          nationalController.text.length <=
                                              12));
                                  provider.updatePhoneErr(
                                    !(phoneController.text.isNotEmpty &&
                                        StringUtils.instance
